...
const defaultTeamData = (window.TEAM_DATA && Array.isArray(window.TEAM_DATA)) ? window.TEAM_DATA : [
  {group:'Les petits mots des intervenants', type:'intervenante', name:'Intervenante', role:'Aide au quotidien', initial:'I', photo:'', text:'J’accompagne les bénéficiaires dans les actes essentiels de la vie quotidienne : aide au lever, au coucher, à l’habillage, aux repas et présence rassurante.'},
  {group:'Les petits mots des intervenants', type:'intervenante', name:'Intervenante', role:'Aide au quotidien', initial:'I', photo:'', text:'J’interviens pour l’entretien courant du logement, le linge, le repassage et les petites tâches quotidiennes afin de préserver un cadre de vie agréable.'},
  {group:'Les petits mots des intervenants', type:'intervenante', name:'Intervenante', role:'Aide au quotidien', initial:'I', photo:'', text:'J’accompagne les bénéficiaires aux courses, aux rendez-vous, en promenade ou lors de sorties pour maintenir l’autonomie et le lien social.'},
  {group:'Les petits mots des intervenants', type:'intervenante', name:'Intervenante', role:'Aide au quotidien', initial:'I', photo:'', text:'Je participe au bien-être des bénéficiaires avec une attention particulière lors des passages à domicile, notamment autour des repas et des besoins du quotidien.'},
  ...
  return groups.map(group => {
    ...
    const cards = people.map(p => `
      <article class="team-person ${p.type === 'bureau' || p.type === 'president' ? 'strong' : ''}">
        ${photoOrInitial(p)}
        <h4 class="team-name-line"><span>${p.name}</span><small>${p.role}</small></h4>
        <p>${p.text}</p>
      </article>
    `).join('');
...
// AJOUT : affichage du flyer portage de repas au clic
const flyerPortageModal = document.getElementById('flyerPortageModal');
const flyerOpenButtons = [
  document.getElementById('openFlyerPortage'),
  document.getElementById('openFlyerPortagePreview')
].filter(Boolean);
function openFlyerPortage(){
  if(!flyerPortageModal) return;
  flyerPortageModal.classList.add('open');
  flyerPortageModal.setAttribute('aria-hidden','false');
  document.body.classList.add('modal-open');
}
function closeFlyerPortage(){
  if(!flyerPortageModal) return;
  flyerPortageModal.classList.remove('open');
  flyerPortageModal.setAttribute('aria-hidden','true');
  document.body.classList.remove('modal-open');
}
flyerOpenButtons.forEach(btn => btn.addEventListener('click', openFlyerPortage));
document.querySelectorAll('[data-close-flyer]').forEach(el => el.addEventListener('click', closeFlyerPortage));
document.addEventListener('keydown', e => { if(e.key === 'Escape') closeFlyerPortage(); });
